...
export const ArticleForm = ({editMode})=>{
   ...
   return (
      ...
      <fieldset className={styles.tags}>
        ...
        {fields.map((tag, index, arr) => (
         ...
         {arr.length - 1 === index ? (
              <Button className={styles.tagButton} type="primary"  ghost onClick={() => append({ tag: '' })}>
                Add tag
              </Button>
            ) : (
              <Button className={styles.tagButton} type="primary" danger ghost onClick={() => remove(index)}>
                Delete
              </Button>
            )}
          </div>
